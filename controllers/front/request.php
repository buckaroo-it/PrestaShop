...
class Buckaroo3RequestModuleFrontController extends BuckarooCommonController
{
    ...
    public function postProcess()
    {
        ...
        // var_dump(PaymentModule::getInstalledPaymentModules());
        //     var_dump('<br>');
        if (!$authorized) {
        ...
        // $this->checkout->returnUrl = 'http' . ((Tools::getIsset($_SERVER["HTTPS"]) && $_SERVER["HTTPS"] == "on") ? 's' : '') . '://' . $_SERVER["SERVER_NAME"] . __PS_BASE_URI__ . 'index.php?controller=order-confirmation&id_cart=' . $cart->id . '&id_module=' . $this->module->id . '&id_order=' . $this->module->currentOrder . '&key=' . $customer->secure_key;
