...
function buckaroo() {
    ...
    class BuckarooGooglePay {
        ...
        init() {
            console.log('testtt');
            if (typeof buckarooGooglePayConfig === 'undefined' || !buckarooGooglePayConfig || !buckarooGooglePayConfig.enabled) {
                return;
            }

            if (typeof BuckarooSdk !== 'undefined' && BuckarooSdk.GooglePay && BuckarooSdk.GooglePay.GooglePayPayment) {
                this.sdkLoaded = true;
                this.renderButton();
                return;
            }
        ...
        loadSdk() {
            if (this.sdkLoaded || this.sdkLoading) {
                return;
            }
            ...
            script.onerror = () => {
                this.sdkLoading = false;
                console.error('Failed to load Buckaroo SDK for Google Pay');
            };
        ...
        renderButton() {
            ...
            const containerId = buckarooGooglePayConfig.buttonContainerId || 'google-pay-button-container';
            let container = document.getElementById(containerId);

            if (!container) {
                // Try to create a container inside the Google Pay payment option (if present)
                const googlePayOption = document.querySelector('[data-module-name="googlepay"]')
                    || document.querySelector('[id*="googlepay"]');

                if (googlePayOption) {
                    const paymentOption = googlePayOption.closest('.payment-option');
                    const additionalInfo = paymentOption ? paymentOption.nextElementSibling : null;

                    if (additionalInfo) {
                        container = document.createElement('div');
                        container.id = containerId;
                        additionalInfo.appendChild(container);
                    }
                }
            }
            ...
            const cfg = buckarooGooglePayConfig;

            const googlePayPayment = new BuckarooSdk.GooglePay.GooglePayPayment({
                environment: cfg.environment || 'TEST',
                allowedCardNetworks: cfg.allowedCardNetworks || undefined,
                allowedCardAuthMethods: cfg.allowedCardAuthMethods || undefined,
                buttonColor: cfg.buttonColor || undefined,
                buttonType: cfg.buttonType || undefined,
                buttonRadius: cfg.buttonRadius || 4,
                buttonLocale: cfg.buttonLocale || undefined,
                buttonContainerId: container.id,
                totalPriceStatus: cfg.totalPriceStatus || 'FINAL',
                totalPrice: String(cfg.totalPrice || ''),
                ...
                onGooglePayLoadError: (error) => {
                    console.error('Error loading Google Pay button:', error);
                },
                processPayment: (paymentData) => {
                    try {
                        const tokenObject = paymentData && paymentData.paymentMethodData
                            && paymentData.paymentMethodData.tokenizationData
                            && paymentData.paymentMethodData.tokenizationData.token;

                        const cardInfo = paymentData && paymentData.paymentMethodData && paymentData.paymentMethodData.info;
                        const customerCardName = cardInfo && (cardInfo.cardholderName
                            || (cardInfo.billingAddress && cardInfo.billingAddress.name)) || '';

                        if (!tokenObject) {
                            console.error('Google Pay token missing in paymentData');
                            return;
                        }

                        // Submit POST form to Buckaroo request controller
                        const form = document.createElement('form');
                        form.method = 'POST';
                        form.action = buckarooGooglePayConfig.requestUrl
                            || '/index.php?fc=module&module=buckaroo3&controller=request';

                        const methodInput = document.createElement('input');
                        methodInput.type = 'hidden';
                        methodInput.name = 'method';
                        methodInput.value = 'googlepay';
                        form.appendChild(methodInput);

                        const tokenInput = document.createElement('input');
                        tokenInput.type = 'hidden';
                        tokenInput.name = 'googlePayToken';
                        tokenInput.value = typeof tokenObject === 'string'
                            ? tokenObject
                        ...
                        if (customerCardName) {
                            ...
                            nameInput.name = 'googlePayCardName';
                            nameInput.value = customerCardName;
                            form.appendChild(nameInput);
                        }

                        if (typeof buckarooGooglePayConfig.csrfToken === 'string' && buckarooGooglePayConfig.csrfToken.length) {
                            const csrfInput = document.createElement('input');
                            csrfInput.type = 'hidden';
                            csrfInput.name = 'token';
                            csrfInput.value = buckarooGooglePayConfig.csrfToken;
                        ...
                        console.error('Error during Google Pay processPayment handling:', e);
                    }
                },
            });
