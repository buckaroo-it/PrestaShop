<template>
    <div class="border-b h-16 flex justify-between items-center">
        <div class="px-5 space-y-1 flex items-center  space-x-3">
            <div class="w-8">
                <img v-if="payment.icon" :src="`${baseUrl}/modules/buckaroo3/views/img/buckaroo/Identification methods/SVG/${ payment.icon }`" alt="" />
            </div>

            <div>
                <h2 class="font-bold h2-title">{{ $t(`verification_methods.${ payment.name }`) }}</h2>
...
export default {
    name: "DefaultVerificationConfig.vue",
    props: ['payment'],
    components: {
        CountrySelect,
    },
