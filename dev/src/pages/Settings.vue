        ...
        <div class="py-5 space-y-5">
          <div class="flex justify-between items-center">
            ...
            <div class="flex flex-col items-end space-y-1" v-if="!testCredentialsApi.loading.value">
              ...
              <div v-if="credentialsAreValid === true" class="text-xs text-green-600">
                {{ $t('dashboard.pages.settings.successfully_verified_the_credentials') }}
              </div>
              <div v-if="credentialsAreValid === false" class="text-xs text-red-600">
          ...
          <div class="space-y-5">
            <div class="relative">
              ...
              <button @click="showWebsiteKey = !showWebsiteKey"
                      class="right-3 absolute top-2 hover:bg-gray-200 text-gray-500 p-1 rounded">
            ...
            <div class="relative">
              ...
              <button @click="showSecretKey = !showSecretKey"
                      class="right-3 absolute top-2 hover:bg-gray-200 text-gray-500 p-1 rounded">
...
<!--                  </div>-->
              ...
              <div class="p-5 space-y-5">
                <div>
                  <h2 class="font-semibold text-sm">{{ $t('dashboard.pages.settings.payment_fee_percentage_mode') }}</h2>
                  <div class="text-gray-400 text-xs" v-html="$t('dashboard.pages.settings.payment_fee_percentage_mode_description')"></div>
                </div>
                <div class="flex rounded shadow border justify-between md:w-80 w-full overflow-hidden font-bold md:text-sm text-xs">
                  <button
                      class="w-1/2 h-12 space-x-1 hover:bg-green-500 hover:text-white"
                      :class="{ 'bg-green-500 text-white': settings.payment_fee_mode === 'subtotal' }"
                      @click="settings.payment_fee_mode = 'subtotal'"
                  >
                    <i v-if="settings.payment_fee_mode === 'subtotal'" class="fas fa-check"></i>
                  ...
                  >
                    ...
                    <span>{{ $t('dashboard.pages.settings.subtotal_incl_tax') }}</span>
                  </button>
                </div>
              </div>
              ...
              <div class="p-5 space-y-5">
                <div>
                  <label for="payment_fee_frontend_label" class="font-semibold text-sm">
                    {{ $t('dashboard.pages.settings.payment_fee_frontend_label') }}
                  </label>
                  <div class="text-gray-400 text-xs">{{ $t('dashboard.pages.settings.payment_fee_frontend_label_description') }}</div>
                </div>
                <div class="relative">
                  <input
                      type="text"
    ...
    <div class="lg:w-1/5 w-full bg-thirdly p-5 space-y-5 text-gray-600 border-l-2 border-gray-100 lg:h-full">
      ...
      <iframe class="rounded-xl" width="100%" height="180" src="https://www.youtube.com/embed/sAw16-VPkhY"
              title="Buckaroo Smart Checkout" frameborder="0"
              allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
              allowfullscreen></iframe>

      <ol class="font-semibold space-y-3 text-xs">
        <li>
          <div class="inline-block border border-gray-400 rounded w-4 h-4 text-xs text-center mr-1"> 1</div>
          <span v-html="$t('dashboard.pages.settings.step_one')"></span>
        </li>
        <li>
          <div class="inline-block border border-gray-400 rounded w-4 h-4 text-xs text-center mr-1"> 2</div>
          <span v-html="$t('dashboard.pages.settings.step_two')"></span>
        </li>
        <li>
          <div class="inline-block border border-gray-400 rounded w-4 h-4 text-xs text-center mr-1"> 3</div>
          <span v-html="$t('dashboard.pages.settings.step_three')"></span>
        </li>
        <li>
          <div class="inline-block border border-gray-400 rounded w-4 h-4 text-xs text-center mr-1"> 4</div>
          <span v-html="$t('dashboard.pages.settings.step_four')"></span></li>
        <li>
          <div class="inline-block border border-gray-400 rounded w-4 h-4 text-xs text-center mr-1"> 5</div>
          <span v-html="$t('dashboard.pages.settings.step_five')"></span></li>
      </ol>
